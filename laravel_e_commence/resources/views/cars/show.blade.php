@props(['title' => '', 'car' => ''])
{{-- {{dd(array_slice($car->features->getAttributes(), 1));  }} --}}

<x-app-layout title='Show Car'>
    <main>
        <div class="container">
          <h1 class="car-details-page-title">{{ $car->maker->name }} {{ $car->model->name }} - {{ $car->year }}</h1>
          <div class="car-details-region">{{ $car->city->name }} - {{ floor((strtotime(now()) - strtotime($car->published_at))/86400)}} days ago</div>
          ...
          <div class="car-details-content">
            <div class="car-images-and-description">
              ...
              <div class="card car-detailed-description">
                ...
                <ul class="car-specifications">
                  ...
                  @foreach(array_slice($car->features->getAttributes(),1) as $feature => $value)
                    {{-- <h1>{{ ucwords(str_replace('_', ' ', $feature)) }} {{ $value }} </h1> --}}
                    <x-car-specification :feature=$feature :value=$value />
                  @endforeach
            ...
            <div class="car-details card">
              ...
              <a href="tel: +{{ \Illuminate\Support\Str::mask($car->owner->phone, '*', -3)  }}" class="car-details-phone">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                +{{ \Illuminate\Support\Str::mask($car->owner->phone, '*', -3)  }}
